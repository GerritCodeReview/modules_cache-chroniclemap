...
It is important to note that chronicle-map does not support updating persisted
cache size as your needs grow. If your needs evolve and you need more entries,
you'll need to generate a brand new persistent cache (i.e. delete the old one).
...
chronicle-map cache, they are meant _just_ for that. It is _highly_ discouraged
to use these in a production environment.
...
than 1000 logged-in users and no more than 1000 medium sized changes.

Information on which values each cache is actually initialized with, can be found
in the `error_log` at start time, in particular two values are also logged to
