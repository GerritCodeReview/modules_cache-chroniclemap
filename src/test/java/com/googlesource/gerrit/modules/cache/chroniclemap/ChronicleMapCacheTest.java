...
public class ChronicleMapCacheTest {
  ...
  @Test
  public void shouldPruneValuesCreatedByADifferentCacheVersion() throws Exception {
    gerritConfig.setString("cache", null, "directory", "cache");
    gerritConfig.save();
    final ChronicleMapCacheImpl<String, String> cacheV1 = newCacheVersion(1);
    cacheV1.put("foo1", "Value1 created by cache v1");
    cacheV1.put("foo2", "Value2 created by cache v1");
    cacheV1.close();

    final ChronicleMapCacheImpl<String, String> cacheV2 = newCacheVersion(2);
    final String foo3Value = "Value3 created by cache v2";
    cacheV2.put("foo3", foo3Value);
    cacheV2.prune();

    assertThat(cacheV2.size()).isEqualTo(1);
    assertThat(cacheV2.get("foo3")).isEqualTo(foo3Value);
  }
