...
not configured in `gerrit.config` and thus are relying on sub-optimal defaults.
Gerrit admin can monitor `cache/chroniclemap/caches_without_chroniclemap_configuration`
metric (as described in
[Detect caches that rely on sub-optimal defaults](#detect-caches-that-rely-on-sub-optimal-defaults)
section) to detect caches that rely on those defaults.
Note that running `adjust-caches-on-defaults` in case when all caches are configured
doesn't result in any tuning being performed.
...
## Detect caches that rely on sub-optimal defaults
...
* One can use `auto-adjust-caches` command with `adjust-caches-on-defaults` option
  ...
  section for details) in order to provide reasonable defaults for caches in
  question.
