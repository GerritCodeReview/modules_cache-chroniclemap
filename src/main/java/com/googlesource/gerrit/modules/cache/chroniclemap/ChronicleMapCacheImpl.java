...
public class ChronicleMapCacheImpl<K, V> extends AbstractLoadingCache<K, V>
  ...
  ChronicleMapCacheImpl(
    ...
    this.store = mapBuilder.createOrRecoverPersistedTo(config.getPersistedFile());
    ...
    logger.atInfo().log(
        ...
        this.store.remainingAutoResizes(),
        this.store.percentageFreeSpace());

    metrics.registerCallBackMetrics(def.name(), this.store, hotEntries);
  }
  ...
  public V getIfPresent(Object objKey) {
    return getOptional(objKey).map(tv -> tv.getValue()).orElse(null);
  }

  @SuppressWarnings("unchecked")
  Optional<TimedValue<V>> getOptional(Object objKey) {
