...
public class TuneChronicleMapCaches extends SshCommand {
  ...
  private long averageKeySize(
    ...
    progress.beginTask(
        String.format("[%s] calculate average key size", cacheName), concurrentMap.size());
    long keysTotalSize =
        concurrentMap.keySet().stream()
            .mapToInt(
                keyWrapper -> {
                  progress.update(1);
                  return serializedKeyLength(cacheName, keyWrapper);
                })
            .asLongStream()
            .sum();
    progress.endTask();
  ...
  private long averageValueSize(
    ...
    progress.beginTask(
        String.format("[%s] calculate average value size", cacheName), concurrentMap.size());
    ...
    long valuesTotalSize =
        concurrentMap.values().stream()
            .mapToInt(
                timedValue -> {
                  progress.update(1);
                  return serializedValueLength(cacheName, timedValue);
                })
            .asLongStream()
            .sum();
    progress.endTask();
