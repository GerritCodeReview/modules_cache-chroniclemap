...
public class MigrateH2Caches extends H2CacheSshCommand {
  ...
  private static int DEFAULT_SIZE_MULTIPLIER = 3;
  private static int DEFAULT_MAX_BLOAT_FACTOR = 3;
        ...
        if (!stats.isEmpty()) {
          ChronicleMapCacheImpl<?, ?> chronicleMapCache =
              new ChronicleMapCacheImpl<>(
                  in,
                  makeChronicleMapConfig(cacheDir.get(), in, stats),
                  null,
  ...
  private ChronicleMapCacheConfig makeChronicleMapConfig(
      Path cacheDir, PersistentCacheDef<?, ?> in, H2AggregateData stats) {
