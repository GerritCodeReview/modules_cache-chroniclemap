...
public class CachedValueMarshaller<V>
    implements BytesWriter<CachedValue<V>>,
        BytesReader<CachedValue<V>>,
        ReadResolvable<CachedValueMarshaller<V>> {

  private final CacheSerializer<V> serializer;
  private final Duration expireAfterWrite;
  private final CachedValue<V> INVALID = new Invalid<>();

  CachedValueMarshaller(CacheSerializer<V> serializer, Duration expireAfterWrite) {
    this.serializer = serializer;
    this.expireAfterWrite = expireAfterWrite;
  }

  @Override
  public CachedValueMarshaller<V> readResolve() {
    return new CachedValueMarshaller<>(serializer, expireAfterWrite);
  }
