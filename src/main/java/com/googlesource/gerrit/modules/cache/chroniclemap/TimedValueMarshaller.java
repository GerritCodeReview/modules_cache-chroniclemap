...
public class TimedValueMarshaller<V>
  ...
  public TimedValueMarshaller<V> readResolve() {
    Metrics toCopy = Optional.ofNullable(metrics).orElseGet(() -> metricsCache.get(name));
  ...
  private static final Metrics createMetrics(MetricMaker metricMaker, String name) {
