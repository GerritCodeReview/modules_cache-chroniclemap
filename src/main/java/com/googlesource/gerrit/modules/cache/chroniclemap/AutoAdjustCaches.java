...
public class AutoAdjustCaches {
  ...
  protected Config run(@Nullable ProgressMonitor optionalProgressMonitor)
    ...
    for (Map.Entry<String, ChronicleMapCacheImpl<Object, Object>> cache :
...

      {
        ...
        long newMaxEntries = newMaxEntries(cacheName, currCache, progressMonitor);
  ...
  private long newMaxEntries(
      String currCacheName,
      ChronicleMapCacheImpl<Object, Object> currCache,
      ProgressMonitor progressMonitor) {
    long currMaxEntries = currCache.getConfig().getMaxEntries();
    long currFreeEntries = currMaxEntries * currCache.getStore().percentageFreeSpace() / 100;
    long newMaxEntries =
    ...
    progressMonitor.beginTask(
        String.format(
            "[%s] maxEntries=%d freeEntries=%d newMaxEntries=%d",
            currCacheName, currMaxEntries, currFreeEntries, newMaxEntries),
        1);
    progressMonitor.endTask();
